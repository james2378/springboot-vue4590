		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐比赛信息模块(开始) -->
		<div class="warp" v-if="$check_action('/competition_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/competition_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_competition_information = !show_list_competition_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_competition_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 比赛信息推荐列表组件 -->
		                <list_competition_information v-if="show_list_competition_information" :list="list_competition_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_competition_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_competition_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_competition_information" :key="i" @click="to_details('competition_information',o,'competition_information_id')">
		                            <td v-for="(oj,n) in fields_competition_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
				import list_competition_information from "@/components/diy/list_competition_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_competition_information: true,
				list_competition_information: [],
				vm_arr_competition_information: [
					"competition_information_id",
							...
							"reportable_number_of_people",
							"referee_account",
							"referee_name",
							"competition_requirements",
							"introduction_to_judges",
						],
				fields_competition_information: [
										{
						key: "project_number",
						label: "项目编号",
						type: "文本"
						...
						type: "文本"
													...
													{
						key: "competition_poster",
						label: "比赛海报",
						type: "图片"
					},
													{
						key: "competition_classification",
						label: "比赛分类",
						type: "下拉"
					},
													{
						...
						type: "日长"
													...
													{
						key: "number_of_participants_in_the_competition",
						label: "比赛人数",
						type: "数字"
					},
													{
						key: "reportable_number_of_people",
						label: "可报人数",
						type: "数字"
					},
													{
						...
						type: "下寻"
													...
													{
						key: "competition_requirements",
						label: "比赛要求",
						type: "多文本"
					},
													{
						key: "introduction_to_judges",
						label: "裁判简介",
						type: "多文本"
					},
									],
		...
		methods: {
			...
			get_competition_information() {
				let url = "~/api/competition_information/get_list?";
								// url = "~/api/competition_information/get_list?orderby=hits desc";
								this.$get(url, {
									"orderby": "hits desc",
							"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_competition_information = json.result.list;
					}
				})
			},
		...
		mounted() {
						this.get_competition_information();
							this.get_menu();
			this.get_slides();
			this.get_article();
			// this.get_link();
		...
		computed: {
					    list_table_score_information() {
		        var list = this.list_competition_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_competition_information.map((o) => {
		                // 第二个中括号是对象的属性
