<template>
	<div class="diy_details page_competition_information" id="competition_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_competition_information'>
							<div_competition_information :obj="obj"></div_competition_information>
...
<script>
	import div_competition_information from "@/components/diy/div_competition_information.vue";
	...
	export default {
			...
			return{
				...
				query: {
						"project_number": "",
							...
							"competition_time": "",
							"number_of_participants_in_the_competition": 0,
							"reportable_number_of_people": 0,
							"referee_account": 0,
							"referee_name": "",
				...
				list_comment: [],
				form_comment: {
					source_table: "competition_information",
					source_field: "competition_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/competition_information/set?competition_information_id=' + obj["competition_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "competition_information",
							source_field: "competition_information_id",
							source_id: this.obj.competition_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.competition_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "competition_information",
					source_field: "competition_information_id",
					source_id: obj["competition_information_id"],
					user_id
				};
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
													query.title = this.obj.entry_name
														query.img = this.obj.competition_poster
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/competition_information/set?competition_information_id=' + _this.obj["competition_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
