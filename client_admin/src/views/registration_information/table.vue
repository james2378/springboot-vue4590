		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="比赛分类">
									<el-input v-model="query.competition_classification"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/registration_information/table','add') || $check_action('/registration_information/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/registration_information/table','del') || $check_action('/registration_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','number_of_applicants')" min-width="200">
					...
					<el-table-column prop="information_remarks" @sort-change="$sortChange" label="信息备注"
				v-if="user_group == '管理员' || $check_field('get','information_remarks')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/registration_information/table','set') || $check_action('/registration_information/view','set') || $check_action('/registration_information/view','get') || $check_action('/取消报名|录入成绩/table','add') || $check_action('/取消报名|录入成绩/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/registration_information/table','set') || $check_action('/registration_information/view','set') || $check_action('/registration_information/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/cancel_information/view')" v-if="user_group == '管理员' || $check_action('/cancel_information/table','add') || $check_action('/cancel_information/view','add')">
						<span>取消报名</span>
					</el-button>
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/score_information/view')" v-if="user_group == '管理员' || $check_action('/score_information/table','add') || $check_action('/score_information/view','add')">
						<span>录入成绩</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/registration_information/get_list?like=0",
