		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="裁判姓名" prop="referee_name">
												<el-input id="referee_name" v-model="form_sub['referee_name']" placeholder="请输入裁判姓名"
							  v-if="user_group === '管理员' || (form_sub['referee_user_id'] && $check_field('set','referee_name')) || (!form_sub['referee_user_id'] && $check_field('add','referee_name'))"></el-input>
					<div v-else-if="$check_field('get','referee_name')">{{form_sub['referee_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','gender_of_referee') || $check_field('add','gender_of_referee') || $check_field('set','gender_of_referee')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="裁判性别" prop="gender_of_referee">
								<el-select id="gender_of_referee" v-model="form_sub['gender_of_referee']"
						v-if="user_group === '管理员' || (form_sub['referee_user_id'] && $check_field('set','gender_of_referee')) || (!form_sub['referee_user_id'] && $check_field('add','gender_of_referee'))">
						<el-option v-for="o in list_gender_of_referee" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','gender_of_referee')">{{form_sub['gender_of_referee']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','age_of_referee') || $check_field('add','age_of_referee') || $check_field('set','age_of_referee')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="裁判年龄" prop="age_of_referee">
								<el-input-number id="age_of_referee" v-model.number="form_sub['age_of_referee']"
						v-if="user_group === '管理员' || (form_sub['referee_user_id'] && $check_field('set','age_of_referee')) || (!form_sub['referee_user_id'] && $check_field('add','age_of_referee'))"></el-input-number>
					<div v-else-if="$check_field('get','age_of_referee')">{{form_sub['age_of_referee']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
			...
			return {
				...
				disabledObj:{
								"referee_name_isDisabled": false,
										"gender_of_referee_isDisabled": false,
												},

				form_sub: {
								"referee_name":  '', // 裁判姓名
										"gender_of_referee":  '', // 裁判性别
										"age_of_referee":  0, // 裁判年龄
									"examine_state": "未审核",
						    "user_id": 0,
					"referee_user_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["referee_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?referee_user_id=" + form_sub["referee_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/referee_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/referee_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/referee_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/referee_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/referee_user/view','get');
