		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="运动员" prop="athletes">
												<el-input id="athletes" v-model="form_sub['athletes']" placeholder="请输入运动员"
							  v-if="user_group === '管理员' || (form_sub['athlete_users_id'] && $check_field('set','athletes')) || (!form_sub['athlete_users_id'] && $check_field('add','athletes'))"></el-input>
					<div v-else-if="$check_field('get','athletes')">{{form_sub['athletes']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="性别" prop="gender">
						...
						v-if="user_group === '管理员' || (form_sub['athlete_users_id'] && $check_field('set','gender')) || (!form_sub['athlete_users_id'] && $check_field('add','gender'))">
				...
				<el-form-item label="年龄" prop="age">
						...
						v-if="user_group === '管理员' || (form_sub['athlete_users_id'] && $check_field('set','age')) || (!form_sub['athlete_users_id'] && $check_field('add','age'))"></el-input-number>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["athlete_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?athlete_users_id=" + form_sub["athlete_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/athlete_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/athlete_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/athlete_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/athlete_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/athlete_users/view','get');
